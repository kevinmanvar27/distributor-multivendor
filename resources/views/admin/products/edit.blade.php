                                                                ...
                                                                <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapse-{{ $index }}" aria-expanded="false" aria-controls="collapse-{{ $index }}">
                                                                    <div class="d-flex align-items-center w-100 me-3">
                                                                        <div class="variation-header-image me-3">
                                                                            @if($variation->image)
                                                                            <img src="{{ $variation->image->url }}" class="rounded" style="width: 50px; height: 50px; object-fit: cover;" alt="Variation">
                                                                            @else
                                            ...
                                            <div class="mb-4">
                                                ...
                                                <div class="border rounded-3 p-3" id="gallery-photos-container">
                                                    ...
                                                    <div id="gallery-preview" class="d-flex flex-wrap gap-2 mb-3">
                                                        @if($product->galleryPhotos->count() > 0)
                                                            @foreach($product->galleryPhotos as $photo)
                                                                <div class="position-relative gallery-item" data-id="{{ $photo->id }}" draggable="true">
                                                                    <div class="bg-light d-flex align-items-center justify-content-center" style="height: 80px; width: 80px;">
                                                                        <img src="{{ $photo->url }}" class="img-fluid" alt="{{ $photo->name }}" style="max-height: 100%; max-width: 100%; object-fit: cover;" onerror="this.parentElement.innerHTML='<i class=\'fas fa-image text-muted\'></i>'">
                                                                    </div>
                                                                    <button type="button" class="btn btn-danger btn-sm position-absolute top-0 end-0 rounded-circle p-1 remove-gallery-item" data-id="{{ $photo->id }}">
                                                                        <i class="fas fa-times"></i>
                                                                    </button>
                                                                    <input type="hidden" name="product_gallery[]" value="{{ $photo->id }}">
                                                                </div>
                                                            @endforeach
                                                        @endif
                                                    </div>
